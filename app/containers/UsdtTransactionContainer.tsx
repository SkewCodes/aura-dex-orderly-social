...
import { useUserPnlsStore } from '~/store/userPnlsStore';
...
export interface UsdtTransactionActions {
...
interface UsdtTransactionContainerProps {
  children: (data: UsdtTransactionData, actions: UsdtTransactionActions) => React.ReactNode;
}

export default function UsdtTransactionContainer({ children }: UsdtTransactionContainerProps) {
  ...
  useEffect(() => {
    if (user.user_id !== -1) {
      ...
      fetchPnls(user.user_id, startDate.getTime() / 1000, endDate.getTime() / 1000, true);
    }
  }, [user.user_id, dateRange, fetchPnls]);
  ...
  const applyDateFilter = (start: Date, end: Date) => {
    ...
    fetchPnls(
      user.user_id, 
      startWithTime.getTime() / 1000, 
      endWithTime.getTime() / 1000, 
      true
    );
  };
  ...
  // 컨테이너에서 관리하는 데이터
  const data: UsdtTransactionData = {
    pnls,
    isLoading,
    error,
    totalCount,
  ...
  // 컨테이너에서 관리하는 액션
  const actions: UsdtTransactionActions = {
