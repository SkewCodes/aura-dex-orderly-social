...
const IpadTradingStatsItem: React.FC<TradingStatsItemProps> = ({ 
  label, 
  value, 
  subvalue, 
  ...
  return (
    <div className="dc-w-[150px] dc-px-1 dc-h-full dc-flex dc-flex-col dc-justify-center dc-gap-1">
      {label ? (
        <>
          <span className="dc-whitespace-nowrap dc-text-[#898D99] dc-text-10 dc-truncate">{label}</span>
          <span className={`dc-whitespace-nowrap ${valueColor} dc-text-11 dc-truncate`}>{value}</span>
        </>
      ) : (
        <>
          <span className={`dc-whitespace-nowrap ${valueColor} dc-text-11 dc-truncate`}>{value}</span>
          <span className="whitespace-nowrap text-[#898D99] text-10 truncate">{subvalue}</span>
        </>
      )}
    </div>
  );
...
interface IpadTradingStatsProps {
  symbol: string;
}

const IpadTradingStats: React.FC<IpadTradingStatsProps> = ({
  symbol
}) => {
  ...
  const priceChangeColor = parseFloat(tickerData.price24hPcnt) >= 0 ? "text-[#3FB185]" : "text-[#FF2B64]";
  ...
  return (
    <div className="dc-relative" ref={dropdownContainerRef}>
      <div className="no-scrollbar dc-h-44 dc-flex dc-items-center dc-w-full dc-overflow-x-auto">
        <div className="dc-gap-6 dc-h-44 dc-px-8 dc-flex dc-items-center dc-justify-between dc-w-full dc-overflow-x-auto dc-text-sm dc-bg-gradient-to-br dc-from-[#131416]/80 dc-to-[#0A0A0B]/80">
          <div className="dc-w-[150px] dc-px-1 dc-h-full dc-flex dc-flex-col dc-justify-center dc-gap-1 dc-relative" ref={dropdownRef}>
            <div 
              className="hover:dc-bg-white hover:dc-bg-opacity-5 dc-flex dc-items-center dc-justify-between dc-p-1 dc--m-1 dc-transition-colors dc-rounded-md dc-cursor-pointer"
              onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            >
              <div className="dc-flex dc-flex-col dc-gap-1">
                <span className="dc-whitespace-nowrap dc-text-11 dc-text-white dc-truncate">{symbol?.replace("USDT", "")}/USDT</span>
                <span className="dc-whitespace-nowrap dc-text-10 dc-text-[#898D99] dc-truncate">{getCurrentSymbolName()}</span>
              </div>
              <ChevronDownIcon 
                className={`dc-w-15 dc-h-15 dc-text-[#00D4FF] dc-transition-transform dc-duration-200 ${isDropdownOpen ? 'dc-rotate-180' : ''}`} 
              />
            </div>
          </div>
        <span className='dc-border-opacity-6 dc-max-h-32 dc-w-1 dc-h-full dc-border dc-border-white'></span>
        <IpadTradingStatsItem 
          value={formatNumber(tickerData.lastPrice, 2)} 
          subvalue={formatNumber(tickerData.lastPrice, 2)} 
          valueColor="text-[#3FB185]" 
        />
        <IpadTradingStatsItem 
        ...
        <IpadTradingStatsItem 
          ...
          value={
            <>
              <span className="text-[#E1E1E1]">{formattedFundingRate()}</span> / {fundingCountdown}
            </>
          } 
        />
      </div>
    ...
    {isDropdownOpen && (
      <div className="symbol-dropdown dc-absolute dc-top-[52px] dc-left-[8px] dc-mt-2 dc-w-[150px] dc-bg-[#1F2126] dc-border dc-border-[#2A2D34] dc-rounded-md dc-shadow-lg dc-z-50 dc-overflow-hidden">
        {availableSymbols.map((item) => (
          <div
            key={item.symbol}
            className={`dc-px-3 dc-py-2 hover:dc-bg-[#2A2D34] dc-cursor-pointer dc-transition-colors ${
              item.symbol === symbol ? 'dc-bg-[#2A2D34] dc-text-[#C7AD88]' : 'dc-text-white'
            }`}
          ...
          >
            <div className="flex flex-col gap-1">
              ...
              <span className="dc-text-10 dc-text-[#898D99]">{item.name}</span>
            </div>
          </div>
        ))}
      </div>
...
export default IpadTradingStats;